...
(function() {
  var gm = {
    ...
    encounter: {
      gm_running: "State the victory condition before the first roll - players need to know what winning looks like. Confirm your GM fate point pool (1 per PC). Hold the twist for when the outcome feels settled. If a PC fails, offer success at a cost before declaring a flat failure. Popcorn Initiative (FCon p.31): whoever acted last passes the turn to any character who hasn't gone yet. Example: Kira attacks the guard (succeeds) - Kira's player says 'the netrunner goes next.' The netrunner jacks in. Netrunner passes to the guard. One full exchange. Nobody has a fixed slot.",
