...
var MD_FOOTER = MD_FOOTER;

function mdHeader(title, campName) {
  return ['# ' + title, '> *' + campName + '*\n'];
}
...
function mdAspectList(aspects) {
  if (!Array.isArray(aspects)) return [];
  return aspects.map(function(a, i) {
    return '- *' + a + '*' + (i === 0 ? ' - **High Concept**' : i === 1 ? ' - **Weakness**' : '');
  });
}

function mdWinLose(victory, defeat) {
  var lines = [];
  if (victory) lines.push('**\u2713 Victory condition:** ' + victory);
  if (defeat)  lines.push('**\u2717 Defeat condition:** ' + defeat);
  return lines;
}

function toMarkdown(genId, data, campName) {
  if (!genId || !data) return '';
  // mdBoxes() replaced by mdBoxes() helper defined above
  switch (genId) {
    case 'npc_minor': {
      var d = data;
      return [
