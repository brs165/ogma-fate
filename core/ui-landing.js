...
// SPA-03: clean URL slugs for History API navigation
const SPA_PAGES = {
  thelongafter: '/ogma-fate/thelongafter',
  cyberpunk:    '/ogma-fate/cyberpunk',
  fantasy:      '/ogma-fate/fantasy',
  space:        '/ogma-fate/space',
  victorian:    '/ogma-fate/victorian',
  postapoc:     '/ogma-fate/postapoc',
  western:      '/ogma-fate/western',
  dVentiRealm: '/ogma-fate/dVentiRealm',
};
...
const CAMPAIGN_INFO = {
  ...
  dVentiRealm: {genre: 'High Fantasy',     vibes: 'D&D 5e · Pathfinder · Dragon Age',              tones: ['dark','action'],  era: 'swords', hook: 'The Senate collapsed. The Vaults are still here. So is everything sealed inside them.'},
};
...
function LandingApp() {
  ...
  var camps = (function() {
    const arr = Object.values(CAMPAIGNS);
    for (var i = arr.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      let tmp = arr[i]; arr[i] = arr[j]; arr[j] = tmp;
    }
  ...
  return h('div', {id: 'land-crt-wrap', className: 'land-shell'},
    ...
    // ── Topbar — minimal: wordmark · Help · theme toggle ───────────────
      ...
      h('div', {className: 'land-onboard-section'},
        h('div', {className: 'land-worlds-inner'},
          ...
          h('div', {className: 'land-onboard-grid'},
            ...
            h('a', {href: 'help/how-to-use-ogma.html', className: 'land-onboard-card'},
              ...
              h('div', {className: 'land-onboard-text'},
                ...
                h('div', {className: 'land-onboard-desc'}, 'Pick a world, roll generators, keep the results, export. Ready for game night in minutes.')
              ),
              h('span', {className: 'land-onboard-arrow'}, '›')
            ),
