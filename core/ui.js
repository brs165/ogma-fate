...
function Floater(props) {
  ...
  function persistPos(p) {
    if (p) DB.saveSession('floater_pos_' + id, p).catch(function(){});
  }
...
function CountdownTracker(props) {
  ...
  function persist(next) {
    setTracks(next);
    DB.saveSession('countdowns_' + campId, next).catch(function(){});
  }
...
function SessionPackPanel(props) {
  ...
  return h('div', {className: 'sp-panel fade-up'},
    ...
    h(SessionPackCard, {genId: 'scene',     data: pack.scene.data}),
    h(SessionPackCard, {genId: 'npc_major', data: pack.npcMajor.data})
  );
}
...
function CampaignApp(props) {
  ...
  useEffect(function() {
    if (result) {
      ...
      DB.saveSession('fate_last_camp', {id: campId}).catch(function() {});
    }
  }, [result]);
  ...
  // Quick Prep Pack — generates Adventure Seed + Scene Setup + Major NPC (first-session bundle)
  ...
  return h('div', {className: 'app-shell', 'data-gen': activeGen},
    ...
    h('header', {className: 'topbar', role: 'banner'},
      ...
      h('nav', {className: 'topbar-tabs', 'aria-label': 'Main navigation'},
        ...
        h('a', {href: '#', className: 'topbar-tab active', 'aria-current': 'page',
          onClick: function(e) { e.preventDefault(); }},
          h('span', {className: 'topbar-tab-icon', 'aria-hidden': 'true'}, '🎲'),
          h('span', {className: 'topbar-tab-label'}, 'Prep')
        ),
        ...
        h('div', {id: 'result-panel', role: 'region', 'aria-label': sessionPack ? 'Full session prep packet' : (result ? 'Generated ' + gen.label + ' result' : 'Ready to generate ' + gen.label), 'aria-live': 'polite', 'aria-atomic': 'true'},
          h('div', {className: 'result-panel', style: {padding: 0, overflow: 'hidden'}},
            ...
            h('div', {className: 'action-bar', ref: rollBtnRef},
              ...
              h('div', {className: 'action-bar-secondary'},
                ...
                pinnedCards.length > 0 && h('button', {
                  ...
                  title: 'Export ' + pinnedCards.length + ' kept result' + (pinnedCards.length === 1 ? '' : 's'),
                  'aria-label': 'Export session',
                  style: {fontSize: 'var(--text-sm)', padding: '6px 10px', minHeight: 36},
                },
                  h('span', {'aria-hidden':'true'}, '\uD83D\uDCE6'), ' Export Table Prep'
                ),
            ...
            !result && !rolling && !inspireMode && h('div', {
              ...
              // P-06: Quick Prep Pack hero button
              h('button', {
                className: 'btn btn-ghost',
                ...
                h('span', null, 'Quick Prep Pack — seed + scene + NPC')
              )
            ),
    ...
    showHistory && h('div', {className: 'hist-panel'},
      ...
      // Fari / Foundry batch export — parked, see BACKLOG.md parking lot
      */
