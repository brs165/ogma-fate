# Ogma — Role Prompts, Conventions, and Reference

> **To start a new AI session:** Point Claude at `devdocs/claude_bootstrap.md`. That file reconstitutes the entire project.
> **This file contains:** Role-specific session prompts (Parts 2–7), working conventions, lessons learned, one-liner commands, file locations, and the refactor session prompt. Use these when you want a specific specialist to lead.
...
CROSS-SYSTEM CONVERSION PROTOCOL:
  ...
  2. Apply the conversion (see team-roles.md Critical Conversion Maps)
...
Check BACKLOG.md for the current WS-05 D&D bridging audit status before starting.
```
...
Your reference material is devdocs/campaign-inspirations.csv (the canonical
touchstone list) and the Campaign Voice Reference in devdocs/team-roles.md.
...
is Tier 2. Check devdocs/BACKLOG.md before every session — it changes every sprint.
...
Before changing any rules content, look it up in the appropriate SRD. We have been wrong about FCon rules before. The Fate Condensed SRD is the primary authority for Ogma output; the Fate Core and FAE SRDs are available for cross-reference and enrichment. All SRDs are in `/mnt/project/` and can be searched directly. When drawing from Core or FAE, convert to Condensed conventions and flag the conversion explicitly.
...
# Run full QA battery
node --check core/ui.js && node --check core/ui-primitives.js && node --check core/ui-renderers.js && node --check core/ui-modals.js && node --check core/ui-landing.js && node --check core/engine.js && node --check core/db.js && node --check core/intro.js && node devdocs/qa_named.js
...
| Table authoring guide | `devdocs/content-authoring.md` (supersedes the deleted `CONTRIBUTING.md`) |
| FCon SRD (searchable) | `/mnt/project/Fate-Condensed-SRD-CC-BY.html` |
| Fate Core SRD | `/mnt/project/fate-core.md` |
| FAE SRD | `/mnt/project/fate-accelerated-SRD.md` |
| Fate Adversary Toolkit | `/mnt/project/fate-adversary-toolkit.md` |
| Fate System Toolkit | `/mnt/project/fate-system-toolkit-SRD.md` |
| Fate Horror Toolkit | `/mnt/project/fate-horror-toolkit.md` |
| Book of Hanz | `/mnt/project/The-Book-of-Hanz-Fate-Core-Thought-of-the-Day.md` |
| Fate Space | `/mnt/project/fate-space-srd.md` |
...
1. devdocs/BACKLOG.md          - understand current sprint priorities
2. devdocs/architecture.md     - understand why the architecture is what it is
3. devdocs/claude_bootstrap.md  - project context and constraints
...
Round report format (add to devdocs/CHANGELOG.md):
