...
var na08Fail = [];
['thelongafter','cyberpunk','fantasy','space','victorian','postapoc','dVentiRealm'].forEach(function(camp) {
  ...
  try {
    ...
    // New format: array of FariEntity envelopes (2 NPCs + 1 session notes = 3)
    var entities = Array.isArray(parsed) ? parsed : [parsed];
    if (entities.length !== 3) { na08Fail.push(camp+':count='+entities.length); return; }
    if (entities.some(function(e){return e.fariType !== 'character' || e.version !== 4;})) { na08Fail.push(camp+':envelope'); return; }
    if (entities.some(function(e){return !e.entity || e.entity.version !== 4;})) { na08Fail.push(camp+':version'); return; }
    if (entities.some(function(e){return !e.entity || !e.entity.name;})) { na08Fail.push(camp+':name'); return; }
...
(function() {
  var src = fs.readFileSync('devdocs/qa_named.js','utf8');
...
// NA-67: run.html has minimum aria-label coverage (Sprint G accessibility pass)
// React JSX uses 'aria-label': (colon) not aria-label= (equals)
(function() {
  var fs = require('fs');
  var html = fs.readFileSync('campaigns/run.html', 'utf8');
  var count = (html.match(/'aria-label':/g) || []).length;
  assert('NA-67: run.html has >=15 aria-label attributes (accessibility pass)',
...
(function() {
  var run = fs.readFileSync('campaigns/run.html', 'utf8');
  assert('NA-81: createSync function defined in run.html',
    run.includes('function createSync('),
...
(function() {
  ...
  assert('NA-82: persist() wired to sync.broadcastState',
    run.includes('sync.broadcastState(state)'),
...
(function() {
  ...
  assert('NA-83: gmOnly cards filtered in broadcastState',
    run.includes('gmOnly') && run.includes('clean.cards'),
...
(function() {
  ...
  assert('NA-84: createSync guards against missing PartySocket',
    run.includes("typeof PartySocket==='undefined'"),
