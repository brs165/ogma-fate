...
Run from the project root. Node.js required. Current named assertion count: **76** (see `qa_named.js` for live count).
...
- [ ] **Variety Matrix adj+hazard first-word check:** No `*Adj` entry shares its first word (case-insensitive) with any `*Hazard` entry in the same `v` block. E.g. `"Automated"` in `*Adj` + `"automated defence"` in `*Hazard` = collision → produces `"Automated automated defence"`. NA-56 catches this deterministically — run `node devdocs/qa_named.js` before submitting. When adding new adj or hazard entries, scan the paired array for first-word overlaps manually.
