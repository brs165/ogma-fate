...
# ── Optional OG meta URL updater (BL-12) — defaults to brs165/ogma-fate ─────
GH_USER="${1:-brs165}"
GH_REPO="${2:-ogma-fate}"
BASE_URL="https://${GH_USER}.github.io/${GH_REPO}"
echo ""
echo "=== Updating OG meta URLs to ${BASE_URL} ==="
for f in $(find "$ROOT" -name "*.html" -not -path "*/node_modules/*"); do
  if grep -q "github.io" "$f" 2>/dev/null; then
    sed -i "s|https://[a-zA-Z0-9_-]*\.github\.io/[a-zA-Z0-9_-]*|${BASE_URL}|g" "$f"
    echo "  ✓ $(basename $f)"
  fi
done

# ── BL-46: Update about.html download link to current zip filename ────────────
echo ""
echo "=== Updating about.html download link to fate-generator-${NEW_VER}.zip ==="
sed -i \
  -e "s|href=\"fate-generator-[^\"]*\.zip\"|href=\"fate-generator-${NEW_VER}.zip\"|g" \
  -e "s|Download fate-generator-[^<]*\.zip|Download fate-generator-${NEW_VER}.zip|g" \
  -e "s|fate-generator-[^ ]*\.zip · [0-9]* worlds|fate-generator-${NEW_VER}.zip · 6 worlds|g" \
  "$ROOT/about.html"
echo "  ✓ about.html download link"
