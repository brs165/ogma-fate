<script>
  ...
  let { card = null, onClose = null, onReroll = null, onSendToTable = null, onRemoveFromTable = null, onDelete = null, isOnTable = false, campName = '', campId = '', mode = 'prep' } = $props();
          ...
          {#if mode === 'prep'}
            {#if isOnTable}
              ...
              {#if onRemoveFromTable}
                <button class="bd-remove-table" onclick={() => { onRemoveFromTable(card.id); handleClose(); }} title="Remove from play table">○ Remove</button>
              {/if}
            {:else}
              <button class="bd-send-table" onclick={() => { if (onSendToTable) onSendToTable(card); handleClose(); }} title="Put this card on the play table">→ Table</button>
            {/if}
          {/if}
