...
import { ALL_SKILLS, SKILL_LABEL, GENERATORS } from '../data/shared.js';
 ...
 */
export function stressFromRating(r) {
  if (r >= 3) return 6;
  if (r >= 1) return 4;
  return 3;
}
...
export const UNIVERSAL_MERGE_KEYS = [
  'stunts',
...
export const UNIVERSAL_INJECT_KEYS = [
 ...
 */
export function filteredTables(t, prefs) {
  if (!prefs) return t;
...
export function generate(genId, t, partySize, opts, seed) {
 ...
 */
export function toBatchMarkdown(cards, campName) {
...
export function toPlainText(genId, data, campName) {
  if (!genId || !data) return '';
  var LADDER = {4:'Great (+4)', 3:'Good (+3)', 2:'Fair (+2)', 1:'Average (+1)'};
  var GEN_LABELS = {pc:'PLAYER CHARACTER',npc_major:'MAJOR NPC',npc_minor:'MINOR NPC',faction:'FACTION',scene:'SCENE',campaign:'CAMPAIGN',encounter:'ENCOUNTER',seed:'ADVENTURE SEED',compel:'COMPEL',challenge:'CHALLENGE',contest:'CONTEST',consequence:'CONSEQUENCE',complication:'COMPLICATION',backstory:'BACKSTORY',obstacle:'OBSTACLE',countdown:'COUNTDOWN',constraint:'CONSTRAINT'};
  var genLabel = GEN_LABELS[genId] || genId.toUpperCase().replace(/_/g,' ');
...
export function toBatchPlainText(cards, campName) {
  if (!cards || !cards.length) return '';
 ...
 */
export function toOgmaJSON(genId, data, campName, campId) {
  if (!genId || !data) return '';
 ...
 */
export function toBatchOgmaJSON(pinnedCards, campName, campId) {
 ...
 */
export function parseOgmaJSON(str) {
  if (!str) return null;
  try {
