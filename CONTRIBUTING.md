...
├── devdocs/qa_named.js    # Named assertion suite — run before every PR
└── devdocs/               # Architecture, backlog, team process docs
```
...
**World voice:**  
Each campaign has a distinct register. Entries must feel native to *that* world — not interchangeable with another. Check `devdocs/team-roles.md` → Campaign Voice Reference.
...
The project backlog and architecture decisions are in `devdocs/BACKLOG.md` and `devdocs/architecture.md` — good reading before proposing large changes.
